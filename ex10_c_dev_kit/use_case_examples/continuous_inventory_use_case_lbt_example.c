...
/**
 * @file continuous_inventory_use_case_lbt_example.c
...
int main(int argc, char const* const argv[])
{
    ...
    if (ex10_result.error == false)
    {
        ...
        if (ex10_result.error)
        {
            ...
            if (ex10_result.module == Ex10ListenBeforeTalk)
            {
                struct Ex10ListenBeforeTalk const* lbt =
                    get_ex10_listen_before_talk();
                ex10_eprintf(
                    "LBT prevented ramping on channel %ld with RSSI of %d",
                    lbt->get_last_frequency_khz(),
                    lbt->get_last_rssi_measurement());
            }
        }
    }
    else
