...
/**
 * @file access_read_write_use_case_example.c
 * @details This example uses the Ex10TagAccessUseCase to halt on all the tags
 * in the field of view, and then does a write of random data to user memory,
 * then reads back that random data.  This sequence is setup during the halted
 * callback while the LMAC is halted on the tag.
 *
 * The example runs this sequence 5 times just to show that the inventory
                                   ...
                                   struct Ex10Result*            ex10_result)
{
    ...
    *ex10_result               = setup_gen2_write_read_sequence(write_value);
    if (ex10_result->error)
    {
        ex10_ex_eprintf("Failed to write gen2 sequence\n");
        return;
    }
